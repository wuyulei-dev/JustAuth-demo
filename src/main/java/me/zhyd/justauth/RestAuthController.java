...
import me.zhyd.justauth.config.AuthProperties;
...
public class RestAuthController {
    ...
    public void renderAuth(@PathVariable("source") String source, HttpServletResponse response) throws IOException {
        ...
        //获取第三方平台 登录页，用户同意后会跳转到重定向地址，并携带code
...
//                        .redirectUri("http://localhost:9001/oauth/baidu/callback")
                ...
                break;
            case "github":
                AuthConfig authConfig = authProperties.getTirdType().get(source);
                authConfig.setScopes(AuthScopeUtils.getScopes(AuthGithubScope.values()));
                authRequest = new AuthGithubRequest(authConfig, stateRedisCache);
                break;
            case "gitee":
